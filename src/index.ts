...
let fieldDAO = new FieldDAO(database);
let fieldPoolDao = new FieldPoolDAO(database);
...
async function test(){
    ...
    let match1ScoreId = scoreDatabase.createMatch(matchScore);

    let match1Score = scoreDatabase.getMatchResults(await(match1ScoreId));
